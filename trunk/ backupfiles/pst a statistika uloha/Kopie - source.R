...
#secteme pocet lidi, kteri ptisli do stanice A behem prvnich dvou minut provozu 
...
#je 140. Pomoci CLV, hladina testu 0.05
print(t.test(task12Counts, mu = 140))

task13Sum <- sum(task12Counts) #soucet poctu lidi, kteri jeli dopoledne mezi B a C
task13Var <- 140^2 #rozptyl exponencialniho rozdeleni za platnosi hypotezy(1/lambda^2)
#pokud plati hypoteza, ma nasledujici statistika rozdeleni N(0,1)
task13Stat <- ( task13Sum - nAM * 140 ) / ( sqrt(nAM * task13Var ) )
#kriticky interval pro hladinu testu 0.05 =  (-1.96,1.96)
if( abs(task13Stat) > 1.96 ) print(paste("Ukol 1.3 - Statistika = ", task13Stat, " => hypotezu zamitam.", sep = " ", collapse = "" )) else  print(paste("Ukol 1.3 - Statistika = ", task13Stat, " => hypotezu nezamitam.", sep = " ", collapse = "" )) 
...
#ukol 2.2 - odhad stredni doby poctu cestujicich odpoledne mezi stanicemi B a C
task22Counts <- NULL #pocty lidi v jednotlivych odpolednich vlacich mezi stanicemi A,B
trainTime <- 2 #cas odjezdu vlaku (minuty)
#vygenerujeme obsah task12Counts
